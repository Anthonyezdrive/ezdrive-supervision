...
// Manage CPO contracts and their attributes
// ============================================================
...
function ContractTypeBadge({ type }: { type: CpoContract["type"] }) {
  const config: Record<CpoContract["type"], { bg: string; text: string; border: string; dot: string; label: string }> = {
...
export function CpoContractsPage() {
  ...
  const { data: networks } = useQuery<{ id: string; name: string }[]>({
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          console.warn("[CpoContractsPage] cpo_networks fetch error:", error.message);
          return [];
        }
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof EMPTY_CONTRACT) => {
      const { data: result, error } = await supabase.from("cpo_contracts").insert({
        type: data.type,
        name: data.name.trim(),
    ...
    onSuccess: () => {
      ...
      closeModal();
      toastSuccess("Contrat cr\u00e9\u00e9", "Le contrat CPO a \u00e9t\u00e9 ajout\u00e9 avec succ\u00e8s");
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: { id: string } & Partial<typeof EMPTY_CONTRACT>) => {
      const { data: result, error } = await supabase.from("cpo_contracts").update({
        type: data.type,
        name: data.name?.trim(),
        network_id: data.network_id || null,
        country_code: data.country_code?.trim(),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toastSuccess("Contrat supprim\u00e9", "Le contrat CPO a \u00e9t\u00e9 supprim\u00e9");
    },
    onError: (err: Error) => toastError("Erreur", err.message),
  });
  ...
  function openEdit(contract: CpoContract) {
    setEditing(contract);
    setForm({
      type: contract.type,
      name: contract.name,
  ...
  const { data: contracts, isLoading } = useQuery<CpoContract[]>({
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          console.warn("[CpoContractsPage] Table not found:", error.message);
          return [];
        }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-foreground-muted mt-0.5">
            G&eacute;rer les contrats CPO et leurs attributs
          </p>
        </div>
        <button
          onClick={openCreate}
        ...
        <div className="flex flex-col items-center justify-center h-56 bg-surface border border-border rounded-2xl">
          ...
          <p className="text-sm text-foreground-muted mt-1 max-w-sm text-center">
            {search.trim()
              ...
              : "Cr\u00e9ez votre premier contrat CPO pour commencer."}
          </p>
          <button
            onClick={openCreate}
            ...
            <table className="w-full">
              <thead className="border-b border-border">
                <tr>
                  ...
                  <th className={thClass}>R&eacute;seau CPO</th>
      ...
      <SlideOver open={modalOpen} onClose={closeModal} title={editing ? "Modifier le contrat" : "Nouveau contrat CPO"}>
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground-muted mb-1.5">R&eacute;seau CPO</label>
            <select
